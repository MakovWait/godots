...
class CommandParser: 
	...
	func _expect_namespace() -> String:
		if not _grammar.supports_namespace(token):
			_raise_error("Expected any `" + " ".join(_grammar.namespaces()) + "` namespace, got `%s`" % token)
			_next_token()
			return ""
	...
	func _expect_verb(scope: String) -> String:
		if not _grammar.supports_verb(scope, token):
			_raise_error("Expected any `" + " ".join(_grammar.verbs(scope)) + "` verb, got `%s`" % token)
			_next_token()
			return ""
