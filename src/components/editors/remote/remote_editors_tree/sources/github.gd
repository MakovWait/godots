...
class Self:
	extends RemoteEditorsTreeDataSource.I
				...
				(
					...
					. new(
						...
						GithubVersionSourceParseYml.new(
							YmlSourceGithub.new(), GithubAssetSourceDefault.new()
						),
					)
				)
			)
...
class GodotAsset:
	...
	var file_name: String:
		get:
			return browser_download_url.get_file()

	var browser_download_url: String:
		get:
			return _json.get("browser_download_url", "")

	var is_zip: bool:
		get:
			return name.get_extension() == "zip"

	func _init(json: Dictionary) -> void:
		_json = json


class GithubItemBase:
	extends RemoteEditorsTreeDataSource.Item
	var _item: TreeItem
	var _assets: RemoteEditorsTreeDataSource.RemoteAssets
...
class GithubAssetItem:
	extends GithubItemBase
	var _asset: GodotAsset

	func _init(
		item: TreeItem, assets: RemoteEditorsTreeDataSource.RemoteAssets, asset: GodotAsset
	) -> void:
...
class GithubReleaseItem:
	...
	func _init(
		item: TreeItem, assets: RemoteEditorsTreeDataSource.RemoteAssets, release: GodotRelease
	) -> void:
		super._init(item, assets)
		_release = release
...
class GithubVersionItem:
	extends GithubItemBase
	var _version: GithubVersion

	func _init(
		item: TreeItem, assets: RemoteEditorsTreeDataSource.RemoteAssets, version: GithubVersion
	) -> void:
		super._init(item, assets)
		_version = version
	...
	func async_expand(tree: RemoteEditorsTreeDataSource.RemoteTree) -> void:
		...
		for release in releases:
			...
			tree_item.set_meta("delegate", GithubReleaseItem.new(tree_item, _assets, release))
			tree_item.collapsed = true
...
class GithubRootItem:
	extends GithubItemBase
	var _versions_source: GithubVersionSource

	func _init(
		item: TreeItem,
		assets: RemoteEditorsTreeDataSource.RemoteAssets,
		versions_source: GithubVersionSource
	) -> void:
		super._init(item, assets)
...
class GithubFilterTarget:
	extends RemoteEditorsTreeDataSource.FilterTarget
	var _name: String
	...
	func is_for_different_platform(platform_suffixes: Array) -> bool:
		...
		return not platform_suffixes.any(
			func(suffix: String) -> bool: return cached_name.ends_with(suffix)
		)
...
class GithubAssetSourceDefault:
	extends GithubAssetSource
	...
	func async_load(version: String, release: String) -> Array[GodotAsset]:
		...
		var response := await HttpClient.async_http_get(
			url % tag, ["Accept: application/vnd.github.v3+json"]
		)
...
class GithubAssetSourceFileJson:
	extends GithubAssetSource
	...
	func async_load(version: String, release: String) -> Array[GodotAsset]:
		var json: Dictionary = JSON.parse_string(
			FileAccess.open(_file_path, FileAccess.READ).get_as_text()
		)
		var result: Array[GodotAsset] = []
		for asset_json: Dictionary in json.get("assets", []):
			result.append(GodotAsset.new(asset_json))
...
class GithubVersionSourceFileJson:
	extends GithubVersionSource
	var _file_path: String
	var _assets_src: GithubAssetSource
	...
	func async_load() -> Array[GithubVersion]:
		var json: Array = JSON.parse_string(
			FileAccess.open(_file_path, FileAccess.READ).get_as_text()
		)
...
class GithubVersionSourceParseYml:
	extends GithubVersionSource
	var _src: YmlSource
	var _assets_src: GithubAssetSource
