...
func init(godots_releases: GodotsRecentReleases.I, updates_click_callback: Callable) -> void:
	self._godots_releases = godots_releases
	_check_updates()
	_notification_button.pressed.connect(func() -> void: updates_click_callback.call())
