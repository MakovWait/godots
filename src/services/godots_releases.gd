...
class Default:
	...
	func async_load() -> void:
		...
		var check_is_latest := func(_release: Release) -> void:
			if latest.value != null or _release.is_draft or _release.is_prerelease:
				return
			_release._mark_as_latest()
			latest.value = _release

		_data.clear()
		for el: Dictionary in json:
			var _release := Release.new(el)
			_data.append(_release)
			check_is_latest.call(_release)
...
class SrcGithub:
	...
	func _get_json(url: String) -> Variant:
		var response := HttpClient.Response.new(await HttpClient.async_http_get(url, headers))
...
class Release:
	...
	var is_draft: bool:
		get:
			return _json.get("draft", true)

	var is_prerelease: bool:
		get:
			return _json.get("prerelease", true)
